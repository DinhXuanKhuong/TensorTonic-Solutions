...
def lstm_cell(x_t: np.ndarray, h_prev: np.ndarray, C_prev: np.ndarray,
    ...
    # YOUR CODE HERE
    # pass
    f_t = sigmoid(np.concat([h_prev, x_t], axis = -1) @ W_f.T + b_f)
    i_t = sigmoid(np.concat([h_prev, x_t], axis = -1) @ W_i.T + b_i)
    C_tidle = np.tanh(np.concat([h_prev, x_t], axis = -1) @ W_c.T + b_c)
