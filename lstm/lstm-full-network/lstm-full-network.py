...
class LSTM:
    ...
    def forward(self, X: np.ndarray) -> tuple:
        ...
        y = np.zeros((N, T, self.output_dim))
        h_prev = np.zeros((N, self.hidden_dim))
        C_prev = np.zeros((N, self.hidden_dim))
        # h_t = None
        
        for t in range(T):
            x_t = X[:, t, :]
            f_t = sigmoid(np.concat([h_prev, x_t], axis = -1) @ self.W_f.T + self.b_f)
            i_t = sigmoid(np.concat([h_prev, x_t], axis = -1) @ self.W_i.T + self.b_i)
            C_tidle = np.tanh(np.concat([h_prev, x_t], axis = -1) @ self.W_c.T + self.b_c)
        
            o_t = sigmoid(np.concat([h_prev, x_t], axis = -1) @ self.W_o.T + self.b_o)
